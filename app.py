...
def _split_fixed_amount(row, amount_index, id_index, split_amount):
    ...
    parts = []
    num_parts = -(-original_amount // split_amount)
    ...
    for i in range(num_parts):
        ...
        new_amount = (
            split_amount
            if i < num_parts - 1
            else original_amount - split_amount * (num_parts - 1)
        )
...
def _split_equally(row, amount_index, id_index):
    ...
    parts_count = -(-original_amount // THRESHOLD)
    base_id = row[id_index]
...
def index():
    if request.method == "POST":
        ...
        input_path = os.path.join(
            UPLOAD_FOLDER, f"{original_filename}_{unique_suffix}{input_ext}"
        )
        ...
        with open(input_path, "rb") as rawfile:
            rawdata = rawfile.read(2048)
            encoding_guess = chardet.detect(rawdata)["encoding"]

        with open(
            input_path, newline="", encoding=encoding_guess or "utf-8"
            ...
            for row in reader:
                if split_method == "fixed":
                    new_rows = _split_fixed_amount(
                        row, amount_index, id_index, split_value_int
                    )
